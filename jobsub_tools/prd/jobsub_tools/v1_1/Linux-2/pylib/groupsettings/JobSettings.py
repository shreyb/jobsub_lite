...
class JobSettings(object):
	def __init__(self):
		...
		if hasattr(self, 'parser'):
			...
			self.parser = MyParser(usage=usage, version=os.environ.get("SETUP_JOBSUB_TOOLS","NO_UPS_DIR"))
			self.parser.disable_interspersed_args()
			self.generic_group = OptionGroup(self.parser, "Generic Options")
			self.parser.add_option_group(self.generic_group)
			
			self.file_group= OptionGroup(self.parser, "File Options")
			self.parser.add_option_group(self.file_group)

			self.sam_group= OptionGroup(self.parser, "SAM Options")
			self.parser.add_option_group(self.sam_group)
		...
		self.settings['x509userproxy'] = os.environ.get("X509_USER_PROXY")
		if self.settings['x509userproxy'] == None:
			user=os.environ.get("USER")
			group=os.environ.get("GROUP","common")
			if group=="common":
				self.settings['x509userproxy'] = "/scratch/"+user+"/grid/"+user+".proxy"
			else:
				self.settings['x509userproxy'] = "/scratch/"+user+"/grid/"+user+"."+group+".proxy"
	...
	def runParser(self,a=None,b=None):
		(options, args) = self.parser.parse_args(a,b)
...
##							   help="output file  ")
		...
		generic_group.add_option("-c","--condor_requirements", dest="requirements",action="append",
		...
		generic_group.add_option("-x","--X509_USER_PROXY", dest="x509userproxy",action="store",type="string",
	...
	def checkSanity(self):
		...
		if settings['joblogfile'] and settings['queuecount'] > 1:
			err = " -L and -N=%d (where N>1) options are not allowed together " % settings['queuecount']
			raise InitializationError(err)
		...
		if settings['nowrapfile'] or settings['input_tar_file']!="":

			if len(settings['output_dir_array'])>0:
				err = "-d file transfers are done in the wrapfile, using with --nowrapfile or --input_tar_file does not make sense"
				raise InitializationError(err)

			if len(settings['input_dir_array'])>0:
				err = "-f file transfers are done in the wrapfile, using with --nowrapfile or --input_tar_file does not make sense"
				raise InitializationError(err)
			
			if settings['use_gftp']:
				err = "--use_gftp file transfers are done in the wrapfile, using with --nowrapfile input_tar_file does not make sense"
				raise InitializationError(err)
	...
	def makeWrapFilePreamble(self):
		...
		f.write("\n")
		f.write("source /grid/fermiapp/products/common/etc/setups.sh\n")
	...
	def makeSAMBeginFiles(self):
		...
		f.write("ifdh startProject $PRJ_NAME $EXPERIMENT $DEFN  $SAM_USER $EXPERIMENT\n")		   
		f.close()
	...
	def makeSAMEndFiles(self):
		...
		f.write("setup ifdhc\n")
		f.write("""export IFDH_BASE_URI=%s\n"""%settings['ifdh_base_uri'])
		f.write("CPURL=`ifdh findProject $PRJ_NAME $EXPERIMENT`\n")
	...
	def makeCondorFiles2(self,job_iter=0):
		...
		while (retVal == 0):
			...
			if uniquer==1:
				...
				if settings['useparrot']==False and settings['nowrapfile']==False:
	...
	def makeCommandFile(self, job_iter=0 ):
		#print "self.makeCommandFile(%s)"%job_iter
		settings = self.settings
		...
		if settings['grid']:
			...
			if settings['opportunistic']==0:
				...
				if job_iter <= 1:
					settings['requirements']=settings['requirements'] +\
					' && (((TARGET.AGroup=="group_%s") && (GLIDEIN_Site=="FNAL_%s"))||  (GLIDEIN_Site=="FNAL_%s_opportunistic"))'% \
					(settings['group'],settings['group'],settings['group'])
