...
status_strings = {0:"Wait",1:"Idle", 2:"Running", 3:"Removed", 4:"Completed", 5:"Held", 6:"Suspended", 7:"Assigned"}
...
class DAGManLogParser:
    def parseSubmit(self,jid,datetime,other,block,ignore_sections):
        if not ignore_sections:
           ...
           self.jids[jid] = {"section":section,"submit":datetime,"current":status_codes["Idle"],"nr_starts":0,"lost_secs":0,"nr_suspends":0,"nr_holds":0}
           self.sections[section] = jid
        else:
            self.jids[jid] = {"submit":datetime,"current":status_codes["Idle"],"nr_starts":0,"lost_secs":0,"nr_suspends":0,"nr_holds":0}
    ...
    def parseAbort(self,jid,datetime,other,block):
        self.jids[jid]["abort"] = datetime
        if self.jids[jid]["current"]==status_codes["Running"]:
            self.jids[jid]["lost_secs"] = self.jids[jid]["lost_secs"] + (datetime["utime"] - self.jids[jid]["exe"]["utime"])
        self.jids[jid]["current"] = status_codes["Removed"]
    ...
    def mktime(self,datestr,timestr):
        ...
        if (utime>(self.curtime+360000)): # probably the date was in the past year
            utime = time.mktime((year-1,month,day,hour,min,sec,-1,-1,-1))
        return utime
    ...
    def parseBlock(self,block,ignore_sections,utime_only):
        ...
        if utime_only:
            ...
            datetime = {"date":date,"time":time,"utime":self.mktime(date,time)}
    ...
    def parseFile(self,fd,ignore_sections,utime_only):
        ...
        while 1:
            ...
            try:
		#print "calling parseBlock(lines=%s,ignore_sections=%s,utime_only=%s)"%(lines,ignore_sections,utime_only)
                self.parseBlock(lines,ignore_sections,utime_only)
            except TypeError, msg:
                raise Exception, "Lines %i-%i Block %i: %s" % (i,j,block_nr,msg)
            fd.release()
            changed=1
    ...
    def __init__(self,logfilename,ignore_sections=0,utime_only=1,use_cache=0,cache_dir=None):
        if use_cache:
            if cache_dir==None:
              ...
              cache_basename=cache_dir+"/cache_"+string.replace(logfilename,"/","_")
        ...
        try:
            ...
            #will be needed to convert times, since we don't have the year in the log files
            self.curtime = os.stat(logfilename)[stat.ST_MTIME]
            self.curtimelist = time.localtime(self.curtime)
            #print "calling parseFile(lio=%s ignore_sections=%s, utime_only=%s)"%(lio,ignore_sections,utime_only) 
