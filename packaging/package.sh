...
mv ./jobsub ./${VERS}
tar --exclude "test" --exclude "config"  --exclude "jobsub_tools" --exclude="*.pyc" --exclude="*client*" --exclude=".*" --exclude="doc" --exclude="packaging" --exclude="*.log" --exclude="*.sock" --exclude="dev_use_virtual_env.sh" --exclude="Readme" --exclude="setup" --exclude="*.pyo" --exclude="requirements.txt" -cf ${VERS}.tar -v ${VERS}
mv ./${VERS} ./jobsub
gzip ${VERS}.tar
