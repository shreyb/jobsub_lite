...
class JobsResource(object):
    ...
    def index(self, exp_id, job_id=None, **kwargs):
        try:
            user = cherrypy.request.headers.get('Auth-User')
            if user is not None:
                cherrypy.request.app.log.error('user: %s' % user)
                    ...
                    if job_id is None:
                        ...
                        if jobsub_args is not None:
                            ...
                            if jobsub_command is not None:
                                ...
                                cherrypy.request.app.log.error('command_path: %s' % command_path)
                            ...
                            cherrypy.request.app.log.error('command: %s' % command)
