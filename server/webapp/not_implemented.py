...
class NotImplementedResource(object):
    ...
    def index(self,  **kwargs):
        ...
        try:
            ...
            err = 'Exception on NotImplementedResouce.index: %s' % sys.exc_info()[
                1]
    ...
    def default(self, param1, param2=None, param3=None, param4=None, param5=None,  **kwargs):
        ...
        try:
            if cherrypy.request.method == 'GET':
                rc = {'out': 'informational page for %s/%s/%s/%s/%s not implemented %s %s' % (
                    param1, param2, param3, param4, param5, self.__class__.__name__, kwargs)}
            else:
                err = 'Unimplemented method: %s' % cherrypy.request.method
                logger.log(err)
                rc = {'err': err}
        except:
            err = 'Exception on NotImplementedResouce.default: %s' % sys.exc_info()[
                1]
            logger.log(err, traceback=True)
            rc = {'err': err}
