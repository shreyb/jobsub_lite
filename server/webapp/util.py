...
from authutils import mk_temp_fname
...
def invert_gmap(data):
    #in:  https://fermicloud033.fnal.gov:8443/getGridMapFile 
    ...
    for vo_name in data.keys():
        ...
        for itm in vo_dat:
            if itm['userdn'] not in i_dat.keys():
                i_dat[itm['userdn']] = {'volist':[], 'mapped_uname':{'default': itm['mapped_uname']}}
            i_dat[itm['userdn']]['volist'].append(vo_name)
            if itm['mapped_uname'] != i_dat[itm['userdn']]['mapped_uname']['default']:
...
def invert_vo_role_uid_map(data):
    """ in: https://ferry_server/getVORoleMapFile
