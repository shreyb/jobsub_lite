...
class CdfTest(JobTest):
    ...
    def testTimeoutFlag(self):
        self.assertEqual(
            True,
            False,
            'forgot to implement --timeout flag for this class')
    ...
    def testCdfGoodInput(self):
        ...
        self.assertEqual(
            ns.settings['outLocation'],
            'outLocationValue',
            'setting --outLocation Test FAILED')
        ns.runCmdParser(['--sections=1-3', 'some_script'])
        self.assertEqual(
            ns.settings['sectionList'],
            '1-3',
            ' --sections test FAILED')
        ns.checkSanity()
        self.assertEqual(
            ns.settings['firstSection'],
            1,
            'start FAILED after checkSanity')
        self.assertEqual(
            ns.settings['lastSection'],
            3,
            'end FAILED after checkSanity')
