...
BRANCH_NAMES  Comma separated list of branches that needs to be inspected (from jobsub git repository)
...
process_branch () {
    ...
    if [ -n "$INPLACE" ]; then
        echo ""
        echo "Running on local files in jobsub subdirectory ($1)"
        echo ""
    else
        echo ""
    ...
    refactoring_ret="$(echo "$OUTPUT2" | grep 'Refactored ')"

    # Save the output to a file

    save_as=$(echo "${1//\//_}")

    echo "$OUTPUT2" > "$Log_Dir/Futurize_Log_$save_as.txt"

    if [[ 0 -ne 0 || $futurize_ret2 -ne 0 || $refactoring_ret = *[!\ ]* ]]; then
        refactored_files=$(echo "$OUTPUT2" | grep 'RefactoringTool: Refactored ')
        refactored_file_count=$(echo "$refactored_files" | wc -l)

        echo ""
	echo "There are $refactored_file_count files that need to be refactered"

        if [[ 0 -ne 0 || $futurize_ret2 -ne 0 ]]; then
