...
class authutil_tests(unittest.TestCase):
    ...
    def test_x509pair_to_vomsproxy(self):
        ...
        if krb and os.path.exists(krb):
            ...
            authutils.x509pair_to_vomsproxy(
                test_fname, test_fname, test_fname, 'nova', 'Production')
            self.assertTrue('created %s' % test_fname)
    ...
    def test_krb5cc_to_vomsproxy(self):
        ...
        krb = os.environ.get('KRB5CCNAME')
        if krb and os.path.exists(krb):
            authutils.krb5cc_to_vomsproxy(
                krb, test_fname, 'nova', 'Production')
            self.assertTrue('created %s' % test_fname)
    ...
    def test_x509_proxy_fname(self):
        ...
        self.assertEqual(
            fname, '/var/lib/jobsub/creds/proxies/%s/x509cc_%s' %
            (grp, name))
        ...
        dig = hashlib.sha1()
        dig.update(dn)
        tgt_name = '/var/lib/jobsub/creds/proxies/%s/x509cc_%s_%s_%s' % (
            grp, name, role, dig.hexdigest())
