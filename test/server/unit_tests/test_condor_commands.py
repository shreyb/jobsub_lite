...
class condor_commands_tests(unittest.TestCase):
    ...
    def test_constructFilter(self):
        ...
        self.assertIsInstance(up, str)
        self.assertIn(
            """-constraint \'regexp("group_nova.*",AccountingGroup) && True && True && True\'""",
            up)

        up = condor_commands.constructFilter(uid='dbox')
        self.assertIsInstance(up, str)
        self.assertIn(
            """-constraint \'True && Owner=="dbox" && True && True\'""", up)

        up = condor_commands.constructFilter(jobid='333.0@fifebatch1.fnal.gov')
        self.assertIsInstance(up, str)
        self.assertIn(
            """-constraint \'True && True && regexp("fifebatch1.fnal.gov#333.0.*",GlobalJobId) && True\'""",
            up)
        ...
        for k in condor_commands.JOBSTATUS_DICT.keys():
            ...
            self.assertIsInstance(up, str)
            self.assertIn(
                """-constraint 'True && True && True && JobStatus==%s'""" %
                condor_commands.JOBSTATUS_DICT[k], up)

        up = condor_commands.constructFilter(
            acctgroup="nova", jobstatus="held", uid="dbox")
        self.assertIsInstance(up, str)
        self.assertIn(
            """-constraint \'regexp("group_nova.*",AccountingGroup) && Owner=="dbox" && True && JobStatus==5\'""",
            up)

    @unittest.skip("come back to this one...")
    def test_ui_condor_q(self):
        filt = condor_commands.constructFilter(
            acctgroup="nova", jobstatus="held", uid="dbox")
