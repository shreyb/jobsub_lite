...
scp db.jobsub_client.tar products@$1.fnal.gov:/tmp/jobsub_client/sl7_ups/db
ssh products@$1.fnal.gov "cd /tmp/jobsub_client/sl7_ups/db;  tar xvf db.jobsub_client.tar; rm db.jobsub_client.tar; "
...
mkdir -p jobsub/jobsub_client/ups
mkdir -p jobsub/jobsub_client/__pycache__
cd jobsub
tar cvf prd.jobsub_client.tar jobsub_client

scp prd.jobsub_client.tar products@$1.fnal.gov:/tmp/jobsub_client/sl7_ups/prd
CMD="cd /tmp/jobsub_client/sl7_ups/prd; mkdir -p jobsub_client/$VERS$REV; rm -rf jobsub_client/$VERS$REV/* ; mkdir -p tmp; cd tmp; rm -rf *; tar xvf ../prd.jobsub_client.tar ; cd ..; mv tmp/jobsub_client jobsub_client/$VERS$REV/NULL;mkdir -p jobsub_client/$VERS/NULL/ups; chmod -R g+w jobsub_client/$VERS$REV;rm prd.jobsub_client.tar; rm -rf tmp "
